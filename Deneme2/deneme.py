...
####################################################
###        GMM TABANLI SEGMENTASYON             ###
####################################################

# Apply GMM for segmentation
gmm = GaussianMixture(n_components=3, random_state=42)
reshaped_image = sharpened_2.reshape(-1, 1)
gmm.fit(reshaped_image)
segmented_image = gmm.predict(reshaped_image)
segmented_image = segmented_image.reshape(sharpened_2.shape)

# Convert segmented image to uint8 for further processing
segmented_image_uint8 = (segmented_image * (255 // segmented_image.max())).astype(np.uint8)

# Display GMM Segmented Image
plt.figure(figsize=(8, 6))
plt.title("GMM Segmented Image")
plt.imshow(segmented_image_uint8, cmap='gray')
plt.axis('off')
plt.show()
...
results = {"GMM Segmented Image": segmented_image_uint8}

# Apply convolution for each kernel
for kernel_name, kernel in kernels.items():
    # Convolution
    conv_result = cv2.filter2D(segmented_image_uint8, -1, kernel)
