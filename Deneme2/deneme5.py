...
def preprocess_image(image_path):
    ...
    bilateral_filtered = cv2.bilateralFilter(clahe_equalized, d=5, sigmaColor=75, sigmaSpace=75)
...
def select_roi(image):
    ...
    h, w = image.shape
    x_start = int(w * 0.4)  # ROI alanı optimize edildi
    x_end = int(w * 0.6)
    y_start = int(h * 0.4)
    y_end = int(h * 0.6)
...
def adaptive_threshold(roi):
    ...
    binary_mask = cv2.adaptiveThreshold(roi, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                        cv2.THRESH_BINARY_INV, 11, 2)  # blockSize ve C değerleri optimize edildi
    return binary_mask
...
def refine_and_shape_mask(binary_mask, roi_coords, original_shape):
    ...
    if contours:
        ...
        # Morfolojik işlem: konturu doğrudan çizme
        cv2.drawContours(full_mask[y_start:y_end, x_start:x_end], [largest_contour], -1, 255, thickness=cv2.FILLED)

        # Şekli düzleştirme ve boşlukları doldurma (Morfolojik iyileştirme)
        kernel = np.ones((3, 3), np.uint8)  # Kernel boyutu optimize edildi
