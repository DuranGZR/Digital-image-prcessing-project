...
def refine_and_shape_mask(binary_mask, roi_coords, original_shape):
    ...
    if contours:
        # Belirli bir minimum alan büyüklüğüne sahip konturları seç
        min_area = 800  # Minimum alan büyüklüğü (piksel cinsinden)
        largest_contour = max(contours, key=cv2.contourArea) if contours else None
        filtered_contours = [max(contours, key=cv2.contourArea)] if contours else []
        ...
        # Şekli düzleştir ve bozuklukları gider (Morfolojik operasyonlar: Kapatma ve Açma)
...
def process_image(image_path, ground_truth_path=None):
    ...
    refined_mask = refine_and_shape_mask(thresholded, roi_coords, preprocessed_image.shape)

    # Maskeyi Otsu ve adaptif eşikleme ile birleştir (daha iyi sonuçlar için)
    combined_mask = cv2.bitwise_and(refined_mask, otsu_threshold(preprocessed_image))
    combined_mask = cv2.morphologyEx(combined_mask, cv2.MORPH_CLOSE, np.ones((15, 15), np.uint8), iterations=1)
    combined_mask = cv2.morphologyEx(combined_mask, cv2.MORPH_OPEN, np.ones((5, 5), np.uint8), iterations=2)
    ...
    cv2.imwrite("segmented_output_adaptive_shape.png", combined_mask)
